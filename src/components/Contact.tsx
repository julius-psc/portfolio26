        ...
        <motion.div variants={container} initial="hidden" animate="show" className="mt-6 flex gap-3">
          {links.map((link) => (
            <motion.a
              ...
              className="flex h-10 w-10 items-center justify-center rounded-lg border border-zinc-200 dark:border-zinc-700 hover:border-zinc-300 dark:hover:border-zinc-600 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
            >
              {link.icon === "mail" ? (
                <Mail className="h-5 w-5 text-zinc-600 dark:text-zinc-300" strokeWidth={2} />
              ) : link.icon === "linkedin" ? (
                <img src={Linkedin as unknown as string} alt="LinkedIn" className="h-5 w-5" />
              ) : (
                <img src={Github as unknown as string} alt="GitHub" className="h-5 w-5" />
              )}
            </motion.a>
          ))}
        </motion.div>
